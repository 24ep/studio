...
function createCandidateSummary(candidate) {
    ...
    if (details) {
        if (details.cv_language)
            summaryParts.push(`CV Language: ${details.cv_language}`);
        if (details.personal_info) {
            const pi = details.personal_info;
            if (pi.title_honorific)
                summaryParts.push(`Title: ${pi.title_honorific}`);
            if (pi.nickname)
                summaryParts.push(`Nickname: ${pi.nickname}`);
            if (pi.location)
                summaryParts.push(`Location: ${pi.location}`);
            if (pi.introduction_aboutme)
                summaryParts.push(`About Me: ${pi.introduction_aboutme}`);
        }
        if (details.education && details.education.length > 0) {
            ...
            details.education.forEach((edu, index) => {
                ...
                if (edu.major || edu.field)
                    eduStr += `, Major/Field: ${edu.major || ''}${edu.major && edu.field ? ' / ' : ''}${edu.field || ''}`;
                if (edu.campus)
                    eduStr += `, Campus: ${edu.campus}`;
                if (edu.period)
                    eduStr += `, Period: ${edu.period}`;
                if (edu.duration)
        ...
        if (details.experience && details.experience.length > 0) {
            ...
            details.experience.forEach((exp, index) => {
                ...
                if (exp.postition_level)
                    expStr += ` (Level: ${exp.postition_level})`;
                if (exp.period)
                    expStr += `, Period: ${exp.period}`;
                if (exp.duration)
                    expStr += `, Duration: ${exp.duration}`;
                if (exp.is_current_position)
                    expStr += ` (Current Position)`;
                if (exp.description)
        ...
        if (details.skills && details.skills.length > 0) {
            ...
            details.skills.forEach((skillEntry) => {
        ...
        if (details.job_suitable && details.job_suitable.length > 0) {
            ...
            details.job_suitable.forEach((js, index) => {
                let jsStr = `  ${index + 1}. Career: ${js.suitable_career || 'N/A'}`;
                if (js.suitable_job_position)
                    jsStr += `, Position: ${js.suitable_job_position}`;
                if (js.suitable_job_level)
                    jsStr += `, Level: ${js.suitable_job_level}`;
                if (js.suitable_salary_bath_month)
                    jsStr += `, Salary Expectation (THB/Month): ${js.suitable_salary_bath_month}`;
...
export async function searchCandidatesAIChat(input) {
...
`,
        ...
        return {
            matchedCandidateIds: [],
            aiReasoning: `An unexpected error occurred during AI processing. Details: ${error.message}`
        };
    }
}

...
import { ArrowLeft, Briefcase, CalendarDays, DollarSign, Edit, GraduationCap, HardDrive, Info, ListChecks, Loader2, Mail, MapPin, MessageSquare, Percent, Phone, ServerCrash, Star, Tag, UploadCloud, UserCircle, UserCog, Zap, ExternalLink, Edit3, Save, X, PlusCircle, Trash2, Lightbulb } from 'lucide-react';
...
const positionLevelOptions = ['entry level', 'mid level', 'senior level', 'lead', 'manager', 'executive', 'officer', 'leader'];
...
const RoleSuggestionSummary = ({ candidate, allDbPositions }) => {
    ...
    return (<Card>
      ...
      <CardContent>
        {bestAlternativeMatch && bestAlternativePositionInDb ? (<div className="p-3 border border-dashed border-primary/50 rounded-md bg-primary/5">
            ...
            {currentAppliedPosition ? (<p className="text-xs text-muted-foreground">
                Currently applied for: &quot;{currentAppliedPosition.title}&quot; (Fit Score: {currentFitScore}%)
              </p>) : (<p className="text-xs text-muted-foreground">Currently not formally applied to a specific position in our system (General Fit Score: {currentFitScore}%).</p>)}
            {bestAlternativeMatch.match_reasons && bestAlternativeMatch.match_reasons.length > 0 && (<div className="mt-1.5">
          ...
          </div>) : (<p className="text-sm text-muted-foreground">Candidate appears well-suited for their current applied role, or no significantly stronger alternative open roles were identified from automated matches.</p>)}
      </CardContent>
    </Card>);
};
export default function CandidateDetailPage() {
    ...
    const [allDbPositions, setAllDbPositions] = useState([]);
    const [isEditPositionModalOpen, setIsEditPositionModalOpen] = useState(false);
    const [selectedPositionForEdit, setSelectedPositionForEdit] = useState(null);
    ...
    const fetchCandidateDetails = useCallback(async () => {
        ...
        try {
            ...
            setFetchError(error.message || "Could not load candidate data.");
            setCandidate(null);
        }
        finally {
            setIsLoading(false);
    ...
    const handleUpdateCandidateStatus = async (id, newStatus, notes) => {
        setIsLoading(true);
        try {
    ...
    const handleJobMatchClick = (jobMatchTitle) => {
        if (!jobMatchTitle) {
    ...
    if (fetchError) {
        return (<div className="flex flex-col items-center justify-center h-[calc(100vh-8rem)] text-center p-6">
    ...
    const renderField = (label, value, icon, isLink, linkHref, linkTarget) => {
        ...
        const content = isLink ? (<a href={linkHref} target={linkTarget} rel={linkTarget === "_blank" ? "noopener noreferrer" : undefined} className="text-primary hover:underline cursor-pointer break-all">
        {String(value)}
      </a>) : (<span className="text-foreground break-words">{String(value)}</span>);
        return (<div className="flex items-start text-sm py-1">
    ...
    <div className="space-y-6 p-6">
      <div className="flex justify-between items-center mb-4">
        ...
        {!isEditing && (<Button onClick={() => setIsEditing(true)}><Edit3 className="mr-2 h-4 w-4"/> Edit Details</Button>)}
    ...
    <form onSubmit={handleSubmit(handleSaveDetails)}>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
            ...
            <CardHeader className="flex flex-row items-start justify-between gap-4">
              ...
              <div className="flex flex-col items-end gap-2">
                 {isEditing ? (<>
                        ...
                        <Controller name="status" control={control} render={({ field }) => (<Select onValueChange={field.onChange} value={field.value}>
                                    <SelectTrigger className="text-base px-3 py-1 capitalize"><SelectValue /></SelectTrigger>
                                    <SelectContent>
                                        {availableStages.map(s => (<SelectItem key={s.id} value={s.name}>{s.name}</SelectItem>))}
                                    </SelectContent>
                                </Select>)}/>
          ...
          {!isEditing && (<RoleSuggestionSummary candidate={candidate} allDbPositions={allDbPositions}/>)}
            ...
            <Card>
                ...
                <CardContent>
                    ...
                    {isEditing ? (<div className="space-y-4">
                            {educationFields.map((field, index) => (<div key={field.id} className="p-3 border rounded-md space-y-2 bg-muted/30 relative">
                            ...
                            </ul>) : <div className="text-sm text-muted-foreground text-center py-4">No education details provided.</div>)}
                    </ScrollArea>
                </CardContent>
            </Card>
          ...
          <Card>
              ...
              <CardContent>
                  <ScrollArea className="h-[300px]">
                  {isEditing ? (<div className="space-y-4">
                            {experienceFields.map((field, index) => (<div key={field.id} className="p-3 border rounded-md space-y-2 bg-muted/30 relative">
                            ...
                            </ul>) : <div className="text-sm text-muted-foreground text-center py-4">No experience details provided.</div>)}
                  </ScrollArea>
              </CardContent>
          </Card>
            ...
            <Card>
              ...
              <CardContent>
                <ScrollArea className="h-[300px]">
                {isEditing ? (<div className="space-y-4">
                        {skillsFields.map((field, index) => (<div key={field.id} className="p-3 border rounded-md space-y-2 bg-muted/30 relative">
                        ...
                        </ul>) : <div className="text-sm text-muted-foreground text-center py-4">No skill details provided.</div>)}
                </ScrollArea>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <ScrollArea className="h-[300px]">
                 {isEditing ? (<div className="space-y-4">
                        {jobSuitableFields.map((field, index) => (<div key={field.id} className="p-3 border rounded-md space-y-2 bg-muted/30 relative">
                        ...
                        </ul>) : <div className="text-sm text-muted-foreground text-center py-4">No job suitability details provided.</div>)}
                </ScrollArea>
              </CardContent>
            </Card>
          ...
          <Card>
            ...
            <CardContent>
              {candidate.transitionHistory && candidate.transitionHistory.length > 0 ? (<ScrollArea className="h-[300px]">
                <ul className="space-y-0">
                  {candidate.transitionHistory.sort((a, b) => parseISO(b.date).getTime() - parseISO(a.date).getTime()).map((record, index) => (<li key={record.id} className="p-3 hover:bg-muted/50 transition-colors">
                ...
                </ScrollArea>) : <div className="text-sm text-muted-foreground text-center py-4">No transition history available.</div>}
            </CardContent>
          </Card>
        </div>
              ...
              <CardContent>
                <ScrollArea className="h-[calc(100vh-240px)]">
                  <ul className="space-y-3">
                    {parsed.job_matches.map((match, index) => (<li key={`match-${index}-${match.job_id || index}`} className="p-3 border rounded-md bg-muted/30">
                        <h4 className="font-semibold text-foreground hover:text-primary hover:underline cursor-pointer" onClick={() => handleJobMatchClick(match.job_title)}>
                        ...
                        {match.match_reasons && match.match_reasons.length > 0 && (<div className="mt-1.5">
      ...
      {candidate && (<>
          <UploadResumeModal isOpen={isUploadModalOpen} onOpenChange={setIsUploadModalOpen} candidate={candidate} onUploadSuccess={handleUploadSuccess}/>
          <ManageTransitionsModal candidate={candidate} isOpen={isTransitionsModalOpen} onOpenChange={setIsTransitionsModalOpen} onUpdateCandidate={handleUpdateCandidateStatus} onRefreshCandidateData={fetchCandidateDetails} availableStages={availableStages}/>
        </>)}
      {selectedPositionForEdit && (<EditPositionModal isOpen={isEditPositionModalOpen} onOpenChange={setIsEditPositionModalOpen} position={selectedPositionForEdit} onEditPosition={handlePositionEdited}/>)}
    </div>
    </FormProvider>);
}

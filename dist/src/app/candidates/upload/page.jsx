    ...
    return (<div className="max-w-4xl mx-auto py-10">
      ...
      <Dialog open={dialogOpen} onOpenChange={open => {
        ...
        <DialogContent>
          ...
          {invalidFiles.length > 0 && (<div className="mt-4">
              <h4 className="font-semibold mb-2 text-destructive">Invalid files:</h4>
              <ul className="space-y-1">
                {invalidFiles.map((file, idx) => (<li key={file.name + idx} className="flex items-center text-destructive text-sm">
                    <XCircle className="mr-2 w-4 h-4"/>
                    <span>{file.name} - {file.reason}</span>
                  </li>))}
              </ul>
            </div>)}
          {stagedFiles.length > 0 && (<div className="mt-4">
              <h4 className="font-semibold mb-2">Files to upload:</h4>
              <ul className="space-y-2">
                {stagedFiles.map((file, idx) => (<li key={file.name + file.size + idx} className="flex items-center justify-between bg-muted/50 rounded px-3 py-2">

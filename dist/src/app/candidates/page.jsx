...
async function getInitialCandidatesData(session) {
  ...
  `;
    ...
    try {
        ...
        return { candidates: [], error: error.message || "Failed to load initial candidates." };
    }
}
export default async function CandidatesPageServer() {
    ...
    const [candidatesResult, positionsResult, stagesResult,] = await Promise.allSettled([
        candidatesPromise,
        positionsPromise,
        stagesPromise,
    ]);
    ...
    let candidatesError = undefined;
    let candidatesAuthError = false;
    let candidatesPermissionError = false;
    ...
    if (candidatesAuthError || candidatesPermissionError) {
        return (<CandidateQueueProvider>
        <CandidatesPageClient initialCandidates={[]} initialAvailablePositions={initialPositions} initialAvailableStages={initialStages} authError={candidatesAuthError} permissionError={candidatesPermissionError} initialFetchError={combinedError || undefined}/>
      </CandidateQueueProvider>);
    }
    return (<CandidateQueueProvider>
      <CandidatesPageClient initialCandidates={initialCandidates} initialAvailablePositions={initialPositions} initialAvailableStages={initialStages} initialFetchError={combinedError || undefined}/>
    </CandidateQueueProvider>);
}

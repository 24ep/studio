...
import { PlusCircle, Edit3, Trash2, Save, Loader2, ServerCrash, Settings2 } from 'lucide-react';
...
const customFieldFormSchema = z.object({
    ...
    field_type: z.enum(CUSTOM_FIELD_TYPES, { required_error: "Field type is required" }),
...
export default function CustomFieldsPage() {
    ...
    const handleOpenModal = (definition = null) => {
    ...
    const confirmDelete = (definition) => {
    ...
    if (fetchError) {
        ...
        <p className="text-muted-foreground mb-4 max-w-md">{fetchError}</p>
         {fetchError === "You do not have permission to manage custom field definitions." ? (<Button onClick={() => router.push('/')} className="btn-hover-primary-gradient">Go to Dashboard</Button>) : (<Button onClick={fetchDefinitions} className="btn-hover-primary-gradient">Try Again</Button>)}
      </div>);
    }
    return (<div className="space-y-6 p-6">
      <Card className="shadow-lg">
        ...
        <CardContent className="pt-6">
          {isLoading && definitions.length === 0 ? (<div className="flex justify-center items-center py-10">
                <Loader2 className="h-8 w-8 animate-spin text-primary"/>
                <p className="ml-2 text-muted-foreground">Loading definitions...</p>
            </div>) : definitions.length === 0 && !fetchError ? (<p className="text-muted-foreground text-center py-8">No custom field definitions yet.</p>) : (<div className="border rounded-lg overflow-hidden">
              <Table>
                ...
                <TableBody>
                  {definitions.map((def) => (<TableRow key={def.id}>
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(handleFormSubmit)} className="space-y-4 py-2 pl-1">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <FormField control={form.control} name="model_name" render={({ field }) => (<FormItem>
                  ...
                  <FormField control={form.control} name="field_key" render={({ field }) => (<FormItem>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <FormField control={form.control} name="field_type" render={({ field }) => (<FormItem>
                ...
                if (!['select_single', 'select_multiple'].includes(value)) {
                  ...
                  <FormField control={form.control} name="sort_order" render={({ field }) => (<FormItem>
                ...
                {['select_single', 'select_multiple'].includes(watchFieldType) && (<div className="space-y-3 border p-3 rounded-md">
                    <Label className="text-sm font-medium">Options for Select</Label>
                    {optionsFields.map((optionField, index) => (<div key={optionField.id} className="flex items-end gap-2 p-2 border rounded bg-muted/30">
                        <FormField control={form.control} name={`options.${index}.value`} render={({ field }) => (<FormItem className="flex-1">
                        ...
                        <FormField control={form.control} name={`options.${index}.label`} render={({ field }) => (<FormItem className="flex-1">
                ...
                <FormField control={form.control} name="is_required" render={({ field }) => (<FormItem className="flex flex-row items-center space-x-2 pt-2">

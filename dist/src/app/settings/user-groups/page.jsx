...
import { PlusCircle, Edit3, Trash2, Save, Loader2, ServerCrash, ShieldCheck } from 'lucide-react';
...
export default function RolesPermissionsPage() {
    ...
    const [roles, setRoles] = useState([]); // UserGroups are now "Roles"
    const [selectedRole, setSelectedRole] = useState(null);
    const selectedRoleIdRef = useRef(null); // Ref to store selected role ID
    ...
    const fetchRolesAndSelect = useCallback(async (roleIdToSelect) => {
    ...
    const handleRoleFormSubmit = async (data) => {
        ...
        try {
            ...
            if (!response.ok)
                throw new Error(result.message || `Failed to ${editingRole ? 'update' : 'create'} role`);
    ...
    const handlePermissionToggle = async (permissionId, role) => {
    ...
    return (<div className="space-y-6 p-6">
          ...
          {selectedRole ? (<>
              ...
              <CardContent className="p-4">
                <ScrollArea className="h-[calc(100vh-18rem)]"> {/* Adjust height */}
                  {groupedPermissions.map(group => (<div key={group.category} className="mb-6">
                      <h3 className="text-md font-semibold text-primary mb-2 border-b pb-1">{group.category}</h3>
                      <div className="space-y-3">
                        {group.permissions.map(perm => (<div key={perm.id} className="flex items-center justify-between p-3 rounded-md bg-muted/30 hover:bg-muted/50 transition-colors">
                            <div>
      ...
      {roleToDelete && (<AlertDialog open={!!roleToDelete} onOpenChange={(open) => { if (!open)
            setRoleToDelete(null); }}>
          <AlertDialogContent>
            <AlertDialogHeader>

...
import { Save, BellRing, Info, Loader2, ServerCrash, RefreshCw, Webhook, Mail } from 'lucide-react';
...
export default function NotificationSettingsPage() {
    ...
    const handleSettingChange = (eventId, channelId, field, value) => {
        setEventsWithSettings(prevEvents => prevEvents.map(event => event.id === eventId
            ? {
                ...event,
                channels: event.channels.map(channel => channel.channelId === channelId
                    ? {
                        ...channel,
                        ...(field === 'isEnabled' && { isEnabled: value }),
                        ...(field === 'webhookUrl' && {
                            configuration: { ...channel.configuration, webhookUrl: value },
                        }),
                    }
                    : channel),
            }
            ...
            <div className="space-y-6 p-6">
              {isLoadingData && eventsWithSettings.length === 0 ? (<div className="flex justify-center items-center py-10">
                    ...
                    <div className="space-y-3">
                      {event.channels.map(channel => (<div key={channel.channelId} className="p-3 border rounded-md bg-muted/30">
                          <div className="flex items-center justify-between">
                            ...
                            <Toggle id={`enabled-${event.id}-${channel.channelId}`} checked={channel.isEnabled} onCheckedChange={(checked) => handleSettingChange(event.id, channel.channelId, 'isEnabled', checked)}/>
                          </div>
                          {channel.channelKey === 'webhook' && channel.isEnabled && (<div className="mt-2.5">
                              ...
                              <Input id={`webhookUrl-${event.id}-${channel.channelId}`} type="url" value={channel.configuration?.webhookUrl || ''} onChange={(e) => handleSettingChange(event.id, channel.channelId, 'webhookUrl', e.target.value)} placeholder="https://your-webhook-endpoint.com/..." className="mt-1 text-xs h-8"/>
                            </div>)}
                        </div>))}
                    </div>
                  </Card>)))}

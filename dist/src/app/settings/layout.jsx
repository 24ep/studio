...
import { Settings, Palette, Zap, KanbanSquare, DatabaseZap, Settings2 as CustomFieldsIcon, // Renamed to avoid conflict
...
export const settingsNavItems = [
    ...
    { href: "/settings/system-settings", label: "System Settings", icon: Zap, description: "SMTP, webhooks, and processor concurrency.", permissionId: 'SYSTEM_SETTINGS_MANAGE', adminOnlyOrPermission: true },
...
export default function SettingsLayout({ children }) {
    ...
    return (<div className="flex h-full bg-muted/30">
      <aside className="hidden md:flex md:flex-col md:w-80 border-r bg-card shadow-sm">
        ...
        <ScrollArea className="flex-1">
          <nav className="py-2 px-2">
            {visibleNavItems.map((item) => (<Link key={item.href} href={item.href} className={cn("flex items-start rounded-md px-3 py-2.5 text-sm font-medium transition-colors hover:bg-muted hover:text-primary", pathname === item.href || pathname.startsWith(item.href + '/')
                ? "bg-primary/10 text-primary font-semibold"
                : "text-muted-foreground")}>
                <item.icon className="mr-3 h-5 w-5 shrink-0 mt-0.5"/>
                <div className="flex flex-col">
                  <span>{item.label}</span>
                  <span className={cn("text-xs", pathname === item.href || pathname.startsWith(item.href + '/') ? "text-primary/80" : "text-muted-foreground/80")}>{item.description || ''}</span>
                </div>
              </Link>))}
          </nav>
        </ScrollArea>

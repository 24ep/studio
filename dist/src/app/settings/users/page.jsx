...
import { PlusCircle, UsersRound, ShieldAlert, Edit3, Trash2, ServerCrash, Loader2, MoreHorizontal, Search, XCircle } from "lucide-react";
...
export default function ManageUsersPage() {
    ...
    const fetchUsers = useCallback(async (currentFilters = {}) => {
        ...
        if (currentFilters.role && currentFilters.role !== "ALL_ROLES")
    ...
    if (fetchError && !isLoading) {
        ...
        {fetchError === "You do not have permission to view this page." ? (<Button onClick={() => router.push('/')} className="btn-hover-primary-gradient">Go to Dashboard</Button>) : (<Button onClick={() => fetchUsers({ name: nameFilter, email: emailFilter, role: roleFilter })} className="btn-hover-primary-gradient">Try Again</Button>)}
      </div>);
    }
    return (<div className="space-y-6 p-6">
      <Card className="shadow-lg">
        ...
        <CardContent>
          ...
          <div className="mb-6 p-4 border rounded-lg bg-muted/30 shadow-sm">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
              ...
              <div className="space-y-1">
                ...
                <Select value={roleFilter} onValueChange={(value) => setRoleFilter(value)} disabled={isLoading}>
          ...
          {isLoading && users.length === 0 && !fetchError ? (<div className="text-center py-10">
      ...
      <React.Fragment>
        <AddUserModal isOpen={isAddUserModalOpen} onOpenChange={setIsAddUserModalOpen} onAddUser={handleAddUser}/>
        {selectedUserForEdit && (<EditUserModal isOpen={isEditUserModalOpen} onOpenChange={(isOpen) => {
                setIsEditUserModalOpen(isOpen);
                if (!isOpen)
                    setSelectedUserForEdit(null);
            }} onEditUser={handleEditUser} user={selectedUserForEdit}/>)}
        {userToDelete && (<AlertDialog open={!!userToDelete} onOpenChange={(open) => { if (!open)
            setUserToDelete(null); }}>
            <AlertDialogContent>
              <AlertDialogHeader>

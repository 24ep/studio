...
const UI_DISPLAY_PREFERENCES_SERVER = ["Standard", "Emphasized", "Hidden"];
...
export async function GET(request) {
    ...
    try {
        const result = await getPool().query('SELECT model_type AS "modelType", attribute_key AS "attributeKey", ui_preference AS "uiPreference", custom_note AS "customNote" FROM "UserUIDisplayPreference" WHERE "userId" = $1', [userId]);
        const prefs = {
            candidateAttributes: {},
            positionAttributes: {},
        };
        ...
        await logAudit('ERROR', `Failed to fetch UI preferences for user ${userId}. Error: ${error.message}`, 'API:Preferences:Get', userId);
        return NextResponse.json({ message: "Error fetching user UI preferences", error: error.message }, { status: 500 });
    }
}
export async function POST(request) {
    ...
    if (!validationResult.success) {
        return NextResponse.json({ message: "Invalid input for user UI preferences", errors: validationResult.error.flatten().fieldErrors }, { status: 400 });
    }
    ...
    try {
        ...
        for (const pref of preferencesToSave) {
            await client.query(`INSERT INTO "UserUIDisplayPreference" ("userId", model_type, attribute_key, ui_preference, custom_note, "createdAt", "updatedAt")
         ...
         ON CONFLICT ("userId", model_type, attribute_key) DO UPDATE SET
           ...
           "updatedAt" = NOW()`, [userId, pref.modelType, pref.attributeKey, pref.uiPreference, pref.customNote]);
        }

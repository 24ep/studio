...
export async function POST(request) {
    ...
    try {
        ...
        for (let i = 0; i < stageIds.length; i++) {
            await client.query('UPDATE "RecruitmentStage" SET sort_order = $1 WHERE id = $2', [i, stageIds[i]]);
        }
        await client.query('COMMIT');
        await logAudit('AUDIT', `Recruitment stages reordered by user ${actingUserId}.`, 'API:RecruitmentStages:Reorder', actingUserId, { newOrder: stageIds });
        ...
        await logAudit('ERROR', `Failed to reorder recruitment stages. Error: ${error.message}`, 'API:RecruitmentStages:Reorder', actingUserId, { input: body });

...
import { Settings, Database, HardDrive, Zap, KeyRound, Info, ListChecks, ToggleLeft, ToggleRight, Loader2 } from "lucide-react";
    ...
    const handleCheckMinioBucket = useCallback(async () => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                updateStatusItem('minio_bucket_check', { status: data.status, message: data.message, isLoading: false });
        ...
        <CardContent className="space-y-4">
          ...
          {statuses.map((item) => (<Card key={item.id} className="p-4 shadow-sm bg-card hover:shadow-md transition-shadow">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                ...
                <Badge variant={getStatusBadgeVariant(item.status)} className={cn('self-start sm:self-center whitespace-nowrap', { 'bg-green-500/80 text-primary-foreground': item.status === 'ok' || item.status === 'enabled' }, { 'bg-yellow-400/80 text-secondary-foreground': item.status === 'warning' }, { 'bg-red-500/80 text-destructive-foreground': item.status === 'error' || item.status === 'disabled' })}>
                  {item.status.toUpperCase()}
                </Badge>
              </div>
              <p className="text-sm text-muted-foreground mt-1 ml-7 sm:ml-0">{item.message}</p>
              {item.details && (<div className="mt-2 p-3 bg-muted/50 border border-muted rounded-md text-xs text-muted-foreground ml-7 sm:ml-0">

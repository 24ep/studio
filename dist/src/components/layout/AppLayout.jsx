...
function getPageTitle(pathname) {
    ...
    if (pathname.startsWith("/settings/integrations"))
        return "Integrations";
    if (pathname.startsWith("/settings/stages"))
        return "Recruitment Stages";
    if (pathname.startsWith("/settings/data-models"))
        return "Data Model Preferences";
    if (pathname.startsWith("/settings/custom-fields"))
        return "Custom Field Definitions";
    if (pathname.startsWith("/settings/webhook-mapping"))
        return "Webhook Payload Mapping";
    if (pathname.startsWith("/settings/user-groups"))
        return "User Groups"; // New
    ...
    useEffect(() => {
        ...
        const updateAppConfig = () => {
            if (typeof window !== 'undefined') {
                ...
                [
                    ...
                    if (val)
                        sidebarColors[key] = val;
                });
                setThemeAndColors({ themePreference: themePreference, primaryGradientStart, primaryGradientEnd, sidebarColors });
            }
        };
        ...
        const handleAppConfigChange = (event) => {
            ...
            if (customEvent.detail) {
                ...
                setThemeAndColors({
                    themePreference: (customEvent.detail.themePreference || 'system'),
    ...
    return (<SidebarProvider defaultOpen>
      ...
      <SidebarInset>
        ...
        <main className={isSettingsPage ? "flex-1 overflow-y-auto relative" : "flex-1 overflow-y-auto relative p-6"}>
          {isPageLoading && (<div className="absolute inset-0 flex items-center justify-center bg-background/50 backdrop-blur-sm z-10">

...
export function AddPositionModal({ isOpen, onOpenChange, onAddPosition }) {
      ...
      <DialogContent className="sm:max-w-lg">
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 py-2">
          <div>
            ...
            <Input id="title-add" {...form.register('title')} className="mt-1" placeholder="Enter position title"/>
            {form.formState.errors.title && (<p className="text-sm text-destructive mt-1">{form.formState.errors.title.message}</p>)}
          </div>
          <div>
            <Label htmlFor="department-add">Department *</Label>
            <Input id="department-add" {...form.register('department')} className="mt-1" placeholder="Enter department name"/>

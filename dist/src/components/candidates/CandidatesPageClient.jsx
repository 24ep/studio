...
export function CandidatesPageClient({ initialCandidates, initialAvailablePositions, initialAvailableStages, authError: serverAuthError = false, permissionError: serverPermissionError = false, initialFetchError, }) {
    ...
    const [allCandidates, setAllCandidates] = useState(safeInitialCandidates || []);
    const [availablePositions, setAvailablePositions] = useState(safeInitialAvailablePositions || []);
    const [availableStages, setAvailableStages] = useState(safeInitialAvailableStages || []);
    ...
    const [bulkNewRecruiterId, setBulkNewRecruiterId] = useState(null);
    ...
    const fetchRecruiters = useCallback(async () => {
        ...
        try {
            ...
            toast.error(`Could not load recruiters: ${error.message}`);
        }
    }, [sessionStatus]);
    const fetchPaginatedCandidates = useCallback(async (currentFilters, page, pageSize) => {
        if (sessionStatus !== 'authenticated') {
            setIsLoading(false);
            return;
        }
        ...
        try {
            ...
            if (currentFilters.selectedPositionIds && currentFilters.selectedPositionIds.length > 0)
                query.append('positionId', currentFilters.selectedPositionIds.join(','));
            if (currentFilters.selectedStatuses && currentFilters.selectedStatuses.length > 0)
                query.append('status', currentFilters.selectedStatuses.join(','));
            if (currentFilters.education)
                query.append('education', currentFilters.education);
            if (currentFilters.minFitScore !== undefined)
                query.append('minFitScore', String(currentFilters.minFitScore));
            if (currentFilters.maxFitScore !== undefined)
                query.append('maxFitScore', String(currentFilters.maxFitScore));
            ...
            if (currentFilters.selectedRecruiterIds && currentFilters.selectedRecruiterIds.length > 0)
                query.append('recruiterId', currentFilters.selectedRecruiterIds.join(','));
            ...
            const errorMessage = error.message || "Could not load candidate data.";
    ...
    const refreshCandidateInList = useCallback(async (candidateId) => {
    ...
    const handleUpdateCandidateAPI = async (candidateId, status, transitionNotes) => {
        try {
            ...
            const updatedCandidateFromServer = await response.json();
    ...
    const handleOpenUploadModal = (candidate) => {
    ...
    const handleExportToCsv = async () => {
        ...
        try {
            ...
            if (filters.education)
                query.append('education', filters.education);
            if (filters.minFitScore !== undefined)
                query.append('minFitScore', String(filters.minFitScore));
            if (filters.maxFitScore !== undefined)
                query.append('maxFitScore', String(filters.maxFitScore));
    ...
    const handlePositionEdited = async () => {
        ...
        if (sessionStatus === 'authenticated') {
            ...
            if (posResponse.ok)
                setAvailablePositions(await posResponse.json());
    ...
    const executeBulkAction = async () => {
        if (!bulkActionType || selectedCandidateIds.size === 0)
            return;
        setIsLoading(true);
        try {
            const payload = {
    ...
    return (<div className="flex flex-col md:flex-row gap-6 h-full">
      <aside className="w-full md:w-[20%] flex-shrink-0 md:sticky md:top-[calc(var(--header-height,4rem)_+_1rem)] md:max-h-[calc(100vh-var(--header-height,4rem)-2rem)]">
        <ScrollArea className="h-full md:pr-2">
          ...
          <CandidateFilters initialFilters={filters} onFilterChange={handleFilterChange} onAiSearch={handleAiSearch} availablePositions={availablePositions} availableStages={availableStages} availableRecruiters={availableRecruiters} isLoading={isLoading || isAiSearching} isAiSearching={isAiSearching}/>
        </ScrollArea>
      </aside>
      ...
      <div className="flex-1 md:w-[80%] space-y-6 min-w-0"> 
        <div className="flex flex-col sm:flex-row justify-between items-center gap-2">
          ...
          <div className="w-full md:w-auto flex flex-col sm:flex-row gap-2 items-center sm:justify-end">
            {selectedCandidateIds.size > 0 && canManageCandidates && (<DropdownMenu>
            ...
            {canManageCandidates && (<Button onClick={() => setIsCreateViaAutomationModalOpen(true)} className="w-full sm:w-auto btn-primary-gradient"> <Zap className="mr-2 h-4 w-4"/> Create via Resume </Button>)}
            <DropdownMenu>
        ...
        {aiSearchReasoning && (<Alert variant="default" className="bg-blue-50 border-blue-300 dark:bg-blue-900/30 dark:border-blue-700">
      ...
      {selectedPositionForEdit && (<EditPositionModal isOpen={isEditPositionModalOpen} onOpenChange={(isOpen) => { setIsEditPositionModalOpen(isOpen); if (!isOpen)
        setSelectedPositionForEdit(null); }} position={selectedPositionForEdit} onEditPosition={handlePositionEdited}/>)}
      ...
      <AlertDialog open={isBulkActionConfirmOpen} onOpenChange={setIsBulkActionConfirmOpen}>
        <AlertDialogContent>
          ...
          {bulkActionType === 'assign_recruiter' && (<div className="my-4 space-y-2">

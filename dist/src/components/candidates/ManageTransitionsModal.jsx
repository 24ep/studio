...
import { PlusCircle, CalendarDays, Edit3, Trash2, Save, X, ChevronsUpDown, Check } from 'lucide-react';
...
export function ManageTransitionsModal({ candidate, isOpen, onOpenChange, onUpdateCandidate, onRefreshCandidateData, availableStages, }) {
    ...
    const handleEditNotesClick = (transition) => {
        setEditingTransitionId(transition.id);
        setEditingNotes(transition.notes || '');
    };
    const handleSaveNotes = async (transitionId) => {
        try {
    ...
    const confirmDeleteTransition = (transition) => {
    ...
    return (<>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 py-4">
              ...
              <form onSubmit={form.handleSubmit(handleAddTransitionSubmit)} className="space-y-4">
                ...
                : "Select new stage"}
                        ...
                        <PopoverContent className="w-[--trigger-width] p-0 dropdown-content-height">
                          <div className="p-2">
                            <Input placeholder="Search stage..." value={statusSearchQuery} onChange={(e) => setStatusSearchQuery(e.target.value)} className="h-9"/>
                          </div>
                ...
                <div>
                  ...
                  <Textarea id="notes" placeholder="Add any relevant notes for this transition..." {...form.register('notes')} className="mt-1 min-h-[80px]"/>
                </div>
            ...
            <div>
              ...
              <ScrollArea className="h-[350px] border rounded-md p-1 bg-muted/30">
                {candidate.transitionHistory && candidate.transitionHistory.length > 0 ? (<ul className="space-y-0">
                    {candidate.transitionHistory.sort((a, b) => parseISO(b.date).getTime() - parseISO(a.date).getTime()).map((record, index) => (<li key={record.id} className="p-3 hover:bg-muted/50 transition-colors">
                        <div className="flex items-start space-x-3">
                          ...
                          <div className="flex-1">
                            ...
                            {editingTransitionId === record.id ? (<div className="mt-2 space-y-2">
                                <Textarea value={editingNotes} onChange={(e) => setEditingNotes(e.target.value)} className="text-sm min-h-[60px]" placeholder="Edit notes..."/>
                              ...
                              </div>) : (record.notes && (<p className="text-sm text-foreground mt-1.5 whitespace-pre-wrap">{record.notes}</p>))}
                          </div>
                        </div>
                        {editingTransitionId !== record.id && (<div className="flex justify-end gap-1 mt-1.5">
                  ...
                  </ul>) : (<p className="text-sm text-muted-foreground text-center py-10">No transition history yet.</p>)}
              </ScrollArea>
            </div>
          </div>
      ...
      <AlertDialog open={!!transitionToDelete} onOpenChange={(open) => { if (!open)
        setTransitionToDelete(null); }}>
        <AlertDialogContent>
          <AlertDialogHeader>

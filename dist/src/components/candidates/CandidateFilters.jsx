...
import { Search, Check, ChevronsUpDown, Loader2, CalendarIcon, Brain, Lightbulb } from 'lucide-react';
...
export function CandidateFilters({ initialFilters = {}, onFilterChange, onAiSearch, availablePositions, availableStages, availableRecruiters, isLoading, isAiSearching, aiSearchResults }) {
    const [name, setName] = useState(initialFilters.name || '');
    const [email, setEmail] = useState(initialFilters.email || '');
    const [phone, setPhone] = useState(initialFilters.phone || '');
    const [selectedPositionIds, setSelectedPositionIds] = useState(new Set(initialFilters.selectedPositionIds || []));
    const [selectedStatuses, setSelectedStatuses] = useState(new Set(initialFilters.selectedStatuses || []));
    const [education, setEducation] = useState(initialFilters.education || '');
    const [fitScoreRange, setFitScoreRange] = useState([
        initialFilters.minFitScore || 0,
        initialFilters.maxFitScore || 100,
    ]);
    const [applicationDateRange, setApplicationDateRange] = useState(initialFilters.applicationDateStart && initialFilters.applicationDateEnd
        ? { from: initialFilters.applicationDateStart, to: initialFilters.applicationDateEnd }
        : undefined);
    const [selectedRecruiterIds, setSelectedRecruiterIds] = useState(new Set(initialFilters.selectedRecruiterIds || []));
    const [aiSearchQueryInput, setAiSearchQueryInput] = useState(initialFilters.aiSearchQuery || '');
    const [aiSearchType, setAiSearchType] = useState(initialFilters.aiSearchType || 'hybrid');
    ...
    useEffect(() => {
        ...
        setApplicationDateRange(initialFilters.applicationDateStart && initialFilters.applicationDateEnd
    ...
    const handleAiSearchExample = (example) => {
    ...
    return (<div className="space-y-6">
      ...
      <div className="rounded-lg shadow flex flex-col gap-4  border-primary/20">
        ...
        <Textarea id="ai-search" placeholder="e.g., 'React developers with 5+ years experience at tech companies'" value={aiSearchQueryInput} onChange={(e) => setAiSearchQueryInput(e.target.value)} className="min-h-[80px] text-base" disabled={isLoading || isAiSearching}/>
        <Button onClick={handleAiSearchClick} disabled={!aiSearchQueryInput.trim() || isLoading || isAiSearching} className="w-full md:w-auto mt-2 md:mt-0" size="lg">
          {isAiSearching ? (<>
        ...
        {aiSearchResults && (<Card className="bg-muted/50 mt-2">
      ...
      <div className="grid grid-cols-1 gap-4">
        ...
        <div>
          ...
          <Popover open={positionPopoverOpen} onOpenChange={setPositionPopoverOpen}>
            <PopoverTrigger asChild>
              <Button variant="outline" role="combobox" aria-expanded={positionPopoverOpen} className="w-full justify-between mt-1" disabled={isLoading || isAiSearching}>
            ...
            <PopoverContent className="w-full p-0">
              <Command>
                ...
                <CommandList>
                  ...
                  {filteredPositions.map((position) => (<CommandItem key={position.id} onSelect={() => {
                ...
                setSelectedPositionIds(newSelected);
                      ...
                      <Check className={cn("mr-2 h-4 w-4", selectedPositionIds.has(position.id) ? "opacity-100" : "opacity-0")}/>
                      {position.title}
                    </CommandItem>))}
                </CommandList>
              </Command>
        ...
        <div>
          ...
          <Popover open={statusPopoverOpen} onOpenChange={setStatusPopoverOpen}>
            <PopoverTrigger asChild>
              <Button variant="outline" role="combobox" aria-expanded={statusPopoverOpen} className="w-full justify-between mt-1" disabled={isLoading || isAiSearching}>
            ...
            <PopoverContent className="w-full p-0">
              <Command>
                ...
                <CommandList>
                  ...
                  {filteredStages.map((stage) => (<CommandItem key={stage.name} onSelect={() => {
        ...
        <div>
          ...
          <div className="flex items-center gap-2">
            <Slider value={fitScoreRange} onValueChange={val => setFitScoreRange([val[0], val[1]])} max={100} step={1} className="flex-1" disabled={isLoading || isAiSearching}/>
        ...
        <div>
          ...
          <Popover open={recruiterPopoverOpen} onOpenChange={setRecruiterPopoverOpen}>
            <PopoverTrigger asChild>
              <Button variant="outline" role="combobox" aria-expanded={recruiterPopoverOpen} className="w-full justify-between mt-1" disabled={isLoading || isAiSearching}>
            ...
            <PopoverContent className="w-full p-0">
              <Command>
                ...
                <CommandList>
                  ...
                  {filteredRecruiters.map((recruiter) => (<CommandItem key={recruiter.id} onSelect={() => {
                ...
                setSelectedRecruiterIds(newSelected);
                      ...
                      <Check className={cn("mr-2 h-4 w-4", selectedRecruiterIds.has(recruiter.id) ? "opacity-100" : "opacity-0")}/>
                      {recruiter.name}
                    </CommandItem>))}
                </CommandList>
              </Command>
      ...
      <div className="flex gap-2 mt-4">
        <Button onClick={handleApplyStandardFilters} disabled={isLoading || isAiSearching} className="flex-1">
          <Search className="mr-2 h-4 w-4"/>
          Apply Filters
        </Button>
        <Button variant="outline" size="sm" onClick={handleResetFilters} disabled={isLoading || isAiSearching}>
          Reset All
        </Button>
      </div>
    </div>);

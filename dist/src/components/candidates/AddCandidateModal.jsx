          ...
          <ScrollArea className="flex-grow overflow-y-auto p-6 pt-4">
            <div className="space-y-6">
              ...
              <fieldset className="space-y-3 border p-4 rounded-md">
               ...
               <fieldset className="space-y-3 border p-4 rounded-md">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                     ...
                     <div>
                        ...
                        <Controller name="status" control={form.control} render={({ field }) => (<Select onValueChange={field.onChange} value={field.value}>
                ...
                <div>
                    ...
                    <Controller name="fitScore" control={form.control} render={({ field }) => (<Input id="fitScore" type="number" {...field} onChange={e => field.onChange(parseInt(e.target.value, 10) || 0)} className="mt-1"/>)}/>
                </div>
              </fieldset>

              <fieldset className="space-y-3 border p-4 rounded-md">
                <legend className="text-lg font-semibold">Education</legend>
                {educationFields.map((field, index) => (<div key={field.id} className="p-3 border rounded-md space-y-2 relative bg-muted/30">
              ...
              <fieldset className="space-y-3 border p-4 rounded-md">
                ...
                {experienceFields.map((field, index) => (<div key={field.id} className="p-3 border rounded-md space-y-2 relative bg-muted/30">
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <fieldset className="space-y-3 border p-4 rounded-md">
                    ...
                    {skillFields.map((field, index) => (<div key={field.id} className="p-3 border rounded-md space-y-2 relative bg-muted/30">
                        ...
                        {skillFields.length > 1 && (<Button type="button" variant="ghost" size="icon" className="absolute top-1 right-1 h-7 w-7" onClick={() => removeSkill(index)}>
                                <Trash2 className="h-4 w-4 text-destructive"/>
                            </Button>)}
                    </div>))}
                ...
                <fieldset className="space-y-3 border p-4 rounded-md">
                    <legend className="text-lg font-semibold">Job Suitability</legend>
                     {jobSuitableFields.map((field, index) => (<div key={field.id} className="p-3 border rounded-md space-y-2 relative bg-muted/30">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                             ...
                             {jobSuitableFields.length > 1 && (<Button type="button" variant="ghost" size="icon" className="absolute top-1 right-1 h-7 w-7" onClick={() => removeJobSuitable(index)}>
                                    <Trash2 className="h-4 w-4 text-destructive"/>
                                </Button>)}
                        </div>))}

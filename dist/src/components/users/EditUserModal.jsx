...
import { Save, Loader2, Edit3, Users, ShieldCheck, ChevronsUpDown } from 'lucide-react';
...
const userRoleOptions = ['Admin', 'Recruiter', 'Hiring Manager'];
...
export function EditUserModal({ isOpen, onOpenChange, onEditUser, user, isSelfEdit = false }) {
    const { data: session } = useSession();
    const [activeTab, setActiveTab] = useState(isSelfEdit ? 'general' : 'general');
    ...
    const onSubmit = async (data) => {
        ...
        if (isSelfEdit) {
            delete payload.role;
            delete payload.modulePermissions;
    ...
    return (<Dialog open={isOpen} onOpenChange={(open) => {
      ...
      <DialogContent className="sm:max-w-6xl lg:max-w-7xl xl:max-w-8xl max-h-[95vh] flex flex-col p-0">
        ...
        <div className="flex flex-col lg:flex-row flex-grow overflow-hidden p-8 gap-8">
          {!isSelfEdit && (<aside className="lg:w-80 border-b lg:border-b-0 lg:border-r pb-8 lg:pb-0 lg:pr-8 space-y-2">
              <div className="mb-4">
                <h3 className="text-lg font-semibold text-foreground mb-3">Navigation</h3>
              </div>
              {navItems.map(item => (<Button key={item.id} variant={activeTab === item.id ? "default" : "ghost"} onClick={() => setActiveTab(item.id)} className={cn("w-full justify-start text-base py-3 px-4", activeTab === item.id && "btn-primary-gradient text-primary-foreground shadow-lg")}>
              ...
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                  {activeTab === 'general' && (<div className="space-y-6">
                      ...
                      {!isSelfEdit && (<FormField control={form.control} name="role" render={({ field }) => (<FormItem>
                              ...
                              <Select onValueChange={field.onChange} value={field.value} defaultValue={field.value}>
                                ...
                                <SelectContent>
                                  {userRoleOptions.map(roleValue => (<SelectItem key={roleValue} value={roleValue}>{roleValue}</SelectItem>))}
                                </SelectContent>
                              </Select>
                              <FormMessage />
                      ...
                      {!isSelfEdit && canManageAuthentication && (<div className="space-y-4 p-4 border rounded-lg bg-muted/20">
                          <Label className="text-base font-medium">Authentication Method</Label>
                          <FormField control={form.control} name="authenticationMethod" render={({ field }) => (<FormItem>
                                <FormControl>
                      ...
                      {!isSelfEdit && canForcePasswordChange && form.watch('authenticationMethod') === 'basic' && (<div className="space-y-4 p-4 border rounded-lg bg-muted/20">
                          <div className="flex items-center space-x-2">
                            <FormField control={form.control} name="forcePasswordChange" render={({ field }) => (<FormItem className="flex flex-row items-center space-x-3 space-y-0">
                                  <FormControl>
                          ...
                          {form.watch('forcePasswordChange') && (<FormField control={form.control} name="newPassword" render={({ field }) => (<FormItem>
                                  <FormLabel className="text-base font-medium">New Password *</FormLabel>
                                  <FormControl>
                                    <Input type="password" {...field} placeholder="Enter new password (min 6 characters)" className="h-12 text-base"/>
                                  </FormControl>
                                  <FormMessage />
                      ...
                      {!isSelfEdit && !canForcePasswordChange && form.watch('authenticationMethod') === 'basic' && (<FormField control={form.control} name="newPassword" render={({ field }) => (<FormItem>
                  ...
                  {activeTab === 'permissions' && !isSelfEdit && (<div className="space-y-8">
                          ...
                          <Popover open={groupSearchOpen} onOpenChange={setGroupSearchOpen}>
                            ...
                            <PopoverContent className="w-[--trigger-width] p-0 dropdown-content-height">
                              ...
                              <ScrollArea className="max-h-60">
                                ...
                                {filteredGroups.map(group => (<FormField key={group.id} control={form.control} name="groupIds" render={({ field }) => (<FormItem className="flex flex-row items-center space-x-3 space-y-0 px-3 py-2 hover:bg-accent rounded-md">
                                        <FormControl>
                                          <Checkbox checked={field.value?.includes(group.id)} onCheckedChange={(checked) => checked ? field.onChange([...(field.value || []), group.id]) : field.onChange((field.value || []).filter(v => v !== group.id))}/>
                                        </FormControl>
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="bg-muted/30 rounded-lg border p-6 max-h-96 overflow-y-auto">
                          {groupedPermissions.map(group => (<div key={group.category} className="mb-6 last:mb-0">
                        ...
                        return (<FormItem className="flex flex-row items-start space-x-4 p-3 bg-background rounded-lg border hover:shadow-sm transition-shadow">
                                          <FormControl>
                                            <Toggle checked={checked} onCheckedChange={(checked) => checked ? field.onChange([...(field.value || []), module.id]) : field.onChange((field.value || []).filter(v => v !== module.id))} className="ml-2"/>
                                          </FormControl>

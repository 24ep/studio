...
export default function ApplicationLogsPage() {
    ...
    const fetchLogs = useCallback(async (page, filters) => {
        ...
        try {
            ...
            if (filters.userId !== "ALL")
                url += `&actingUserId=${filters.userId}`;
            if (filters.start)
                url += `&startDate=${filters.start.toISOString()}`;
            if (filters.end)
                url += `&endDate=${filters.end.toISOString()}`;
    ...
    if (fetchError && !isLoading) {
        ...
        {fetchError === "You do not have permission to view logs." ? (<Button onClick={() => router.push('/')} className="btn-hover-primary-gradient">Go to Dashboard</Button>) : (<Button onClick={handleApplyFilters} className="btn-hover-primary-gradient">Try Again</Button>)}
      </div>);
    }
    return (<Card className="shadow-lg">
      ...
      <CardContent>
                  ...
                  <div className="space-y-1">
                    ...
                    <Popover open={userPopoverOpen} onOpenChange={setUserPopoverOpen}>
                      ...
                      <PopoverContent className="w-[--trigger-width] p-0 dropdown-content-height">
                        <Command>
                          <CommandInput placeholder="Search user..." value={userSearch} onChange={e => setUserSearch(e.target.value)} className="h-9"/>
                          <CommandList>
                            ...
                            <ScrollArea className="max-h-48">
                              {filteredUsersForDropdown.map((user) => (<CommandItem key={user.id} value={user.name} onSelect={() => { setActingUserIdFilter(user.id); setUserPopoverOpen(false); setUserSearch(''); }}>
        ...
        {isLoading && logs.length === 0 ? (<div className="flex flex-col items-center justify-center py-10"> <Loader2 className="h-12 w-12 animate-spin text-primary"/> <p className="mt-2 text-muted-foreground">Loading logs...</p> </div>) : logs.length === 0 ? (<div className="text-center py-10"> <ListOrdered className="mx-auto h-12 w-12 text-muted-foreground"/> <p className="mt-4 text-muted-foreground">No log entries found for the selected filter or search query.</p> </div>) : (<>
          <div className="border rounded-lg overflow-hidden">
            <Table>
              ...
              <TableBody>
                {logs.map((log) => (<TableRow key={log.id} className="hover:bg-muted/50 transition-colors">
                    ...
                    <TableCell className="text-xs text-muted-foreground hidden lg:table-cell">
                      {log.actingUserName ? (<div className="flex items-center gap-1.5"> <UserCircle className="h-3.5 w-3.5"/> {log.actingUserName} </div>) : log.actingUserId ? (<div className="flex items-center gap-1.5"> <UserCircle className="h-3.5 w-3.5"/> <span title={log.actingUserId}>User ID: ...{log.actingUserId.slice(-6)}</span> </div>) : 'N/A'}
                    </TableCell>
                  </TableRow>))}
              </TableBody>
            </Table>
          </div>
          {totalPages > 1 && (<div className="flex items-center justify-end space-x-2 py-4">

...
export async function PUT(request) {
    ...
    try {
        ...
        const existingField = await getPool().query('SELECT * FROM "CustomFieldDefinition" WHERE id = $1', [id]);
    ...
    `;
        ...
        await logAudit('AUDIT', `Custom field "${updatedField.label}" (${updatedField.field_key}) updated by ${session.user.name}.`, 'API:CustomFields:Update', session.user.id, { fieldId: id, changes: Object.keys(updateData) });
        return NextResponse.json(updatedField, { status: 200 });
    }
    catch (error) {
        console.error(`Failed to update custom field definition ${id}:`, error);
        await logAudit('ERROR', `Failed to update custom field definition (ID: ${id}). Error: ${error.message}`, 'API:CustomFields:Update', session.user.id, { fieldId: id, input: body });
...
export async function DELETE(request) {
    ...
    `;
        ...
        await logAudit('AUDIT', `Custom field "${fieldData.label}" (${fieldData.field_key}) deleted by ${session.user.name}.`, 'API:CustomFields:Delete', session.user.id, { fieldId: id, modelName: fieldData.model_name, fieldKey: fieldData.field_key });
        ...
        await logAudit('ERROR', `Failed to delete custom field definition (ID: ${id}). Error: ${error.message}`, 'API:CustomFields:Delete', session.user.id, { fieldId: id });

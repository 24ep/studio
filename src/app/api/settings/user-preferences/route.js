...
export async function GET(request) {
    ...
    try {
        const result = await getPool().query('SELECT id, "userId", model_type AS "modelType", attribute_key AS "attributeKey", ui_preference AS "uiPreference", custom_note AS "customNote", "createdAt", "updatedAt" FROM "UserUIDisplayPreference" WHERE "userId" = $1', [userId]);
        return NextResponse.json(result.rows, { status: 200 });
    }
    catch (error) {
        console.error("Failed to fetch user UI preferences:", error);
        await logAudit('ERROR', `Failed to fetch UI preferences for user ${userId}. Error: ${error.message}`, 'API:UserPreferences:Get', userId);

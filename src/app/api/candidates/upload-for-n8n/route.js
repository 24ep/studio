...
export async function POST(request) {
    ...
    const actingUserName = ((_b = session === null || session === void 0 ? void 0 : session.user) === null || _b === void 0 ? void 0 : _b.name) || ((_c = session === null || session === void 0 ? void 0 : session.user) === null || _c === void 0 ? void 0 : _c.email) || 'System (n8n Upload)';
    ...
    try {
        ...
        const targetPositionId = formData.get('positionId');
        const targetPositionDescription = formData.get('targetPositionDescription');
        const targetPositionLevel = formData.get('targetPositionLevel');
        if (!file) {
        ...
        if (actingUserId)
            n8nFormData.append('uploadedByUserId', actingUserId);
        if (actingUserName && actingUserName !== 'System (n8n Upload)')
            n8nFormData.append('uploadedByUserName', actingUserName);
        if (targetPositionId) {
        ...
        await logAudit('ERROR', `Error processing PDF upload for n8n candidate creation by ${actingUserName} (ID: ${actingUserId}). Error: ${error.message}`, 'API:Candidates:N8NCreate', actingUserId);

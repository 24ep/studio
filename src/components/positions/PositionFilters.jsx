...
export function PositionFilters({ initialFilters = { isOpen: "all" }, onFilterChange, isLoading, availableDepartments }) {
    const [title, setTitle] = useState(initialFilters.title || '');
    const [selectedDepartments, setSelectedDepartments] = useState(new Set(initialFilters.selectedDepartments || []));
    const [isOpen, setIsOpen] = useState(initialFilters.isOpen || "all");
    ...
    const renderMultiSelectDepartmentTrigger = () => {
        ...
        if (selectedDepartments.size === 1)
            return Array.from(selectedDepartments)[0];
    ...
    return (<div className="mb-6 p-4 border rounded-lg bg-card shadow">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
        <div>
          <Label htmlFor="title-search">Position Title</Label>
          <Input id="title-search" placeholder="Search by title..." value={title} onChange={(e) => setTitle(e.target.value)} className="mt-1" disabled={isLoading}/>
        </div>
        <div>
            ...
            <PopoverContent className="w-[--trigger-width] p-0 dropdown-content-height">
                <Command>
                    ...
                    <CommandList>
                        ...
                        {filteredDepartments.map((dept) => (<CommandItem key={dept} value={dept} onSelect={() => {
        ...
        <div>
          ...
          <Select value={isOpen} onValueChange={(value) => setIsOpen(value)} disabled={isLoading}>
        ...
        <div>
          ...
          <Input id="level-search" placeholder="Filter by level..." value={positionLevel} onChange={(e) => setPositionLevel(e.target.value)} className="mt-1" disabled={isLoading}/>
        </div>
      </div>

...
import { PlusCircle, Briefcase, Edit, Trash2, ServerCrash, Loader2, FileDown, FileUp, ChevronDown, FileSpreadsheet, Trash2 as BulkTrashIcon, Edit as BulkEditIcon } from "lucide-react";
...
import { AlertDialog, AlertDialogAction, AlertDialogCancel, AlertDialogContent, AlertDialogDescription, AlertDialogFooter, AlertDialogHeader, AlertDialogTitle, AlertDialogTrigger, // Added missing import
...
export default function PositionsPageClient({ initialPositions, initialAvailableDepartments, initialFetchError, authError: serverAuthError = false, permissionError: serverPermissionError = false, }) {
    ...
    const [availableDepartments, setAvailableDepartments] = useState(initialAvailableDepartments || []);
    ...
    const fetchPaginatedPositions = useCallback(async (currentFilters, page, pageSize) => {
        ...
        try {
            ...
            if (activeFilters.title)
                query.append('title', activeFilters.title);
            if (activeFilters.selectedDepartments && activeFilters.selectedDepartments.length > 0)
                query.append('department', activeFilters.selectedDepartments.join(','));
            if (activeFilters.isOpen && activeFilters.isOpen !== "all")
                query.append('isOpen', String(activeFilters.isOpen === "true"));
            if (activeFilters.positionLevel)
                query.append('position_level', activeFilters.positionLevel);
            ...
            setTotal(data.total);
            const uniqueDepts = Array.from(new Set(data.data.map((p) => String(p.department)).filter((d) => typeof d === 'string' && Boolean(d))));
            setAvailableDepartments(uniqueDepts.sort());
        }
        catch (error) {
            const errorMessage = error.message || "Could not load position data.";
    ...
    const handleAddPositionSubmit = async (formData) => {
        try {
    ...
    const handleEditPositionSubmit = async (positionId, data) => {
        try {
    ...
    const handleExportToCsv = async () => {
        ...
        try {
            ...
            if (filters.selectedDepartments && filters.selectedDepartments.length > 0)
                query.append('department', filters.selectedDepartments.join(','));
            if (filters.isOpen && filters.isOpen !== "all")
                query.append('isOpen', String(filters.isOpen === "true"));
            if (filters.positionLevel)
                query.append('position_level', filters.positionLevel);
    ...
    const handleBulkPositionAction = (action) => {
    ...
    const executeBulkPositionAction = async () => {
        if (!bulkActionType || selectedPositionIds.size === 0)
            return;
        setIsLoading(true);
        try {
            ...
            if (!response.ok)
                throw new Error(result.message || 'Bulk position action failed');
    ...
    if (fetchError && !isLoading)
        return ( /* ... (existing unchanged error UI) ... */<div className="flex flex-col items-center justify-center h-[calc(100vh-10rem)] text-center p-4">
    ...
    return (<div className="space-y-6 p-6">
      <div className="flex flex-col sm:flex-row justify-between items-center gap-2">
         <div className="w-full flex flex-col sm:flex-row gap-2 items-center sm:justify-start">
           {selectedPositionIds.size > 0 && canManagePositions && (<DropdownMenu>
         ...
         {canManagePositions && (<Button onClick={() => setIsAddModalOpen(true)} className="w-full sm:w-auto btn-primary-gradient"> <PlusCircle className="mr-2 h-4 w-4"/> Add New Position </Button>)}
      </div>
      ...
      <Card className="shadow-sm">
        ...
        <CardContent>
          {isLoading && positions.length === 0 && !fetchError ? ( /* ... existing JSX for loading ... */<div className="text-center py-10"> <Briefcase className="mx-auto h-12 w-12 text-muted-foreground animate-pulse"/> <p className="mt-4 text-muted-foreground">Loading positions...</p> </div>) : !isLoading && positions.length === 0 && !fetchError ? ( /* ... existing JSX for no positions ... */<div className="text-center py-10"> <Briefcase className="mx-auto h-12 w-12 text-muted-foreground"/> <p className="mt-4 text-muted-foreground">No positions found. Try adjusting filters or add a new position.</p> <Button onClick={() => setIsAddModalOpen(true)} className="mt-4 btn-primary-gradient"> <PlusCircle className="mr-2 h-4 w-4"/> Add New Position </Button> </div>) : (<div className="border rounded-lg overflow-hidden">
            <Table>
              <TableHeader><TableRow><TableHead className="w-12"><Checkbox checked={isAllPositionsSelected} onCheckedChange={handleToggleSelectAllPositions} aria-label="Select all positions" disabled={isLoading || !canManagePositions}/></TableHead><TableHead>Title</TableHead><TableHead className="hidden sm:table-cell">Department</TableHead><TableHead className="hidden md:table-cell">Level</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader>
              <TableBody>
                {positions.map((pos) => (<TableRow key={pos.id} className="hover:bg-muted/50 transition-colors" data-state={selectedPositionIds.has(pos.id) ? "selected" : ""}>
                    <TableCell><Checkbox checked={selectedPositionIds.has(pos.id)} onCheckedChange={() => handleToggleSelectPosition(pos.id)} aria-label={`Select position ${pos.title}`} disabled={isLoading || !canManagePositions}/></TableCell>
      ...
      {canManagePositions && selectedPositionForEdit && (<EditPositionModal isOpen={isEditModalOpen} onOpenChange={(isOpen) => { setIsEditModalOpen(isOpen); if (!isOpen)
        setSelectedPositionForEdit(null); }} onEditPosition={handleEditPositionSubmit} position={selectedPositionForEdit}/>)}
      ...
      <AlertDialog open={isBulkConfirmOpen} onOpenChange={setIsBulkConfirmOpen}>
        <AlertDialogContent>
          ...
          {bulkActionType === 'change_status' && (<div className="my-4 space-y-2">

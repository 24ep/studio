...
export function CandidateTable({ candidates, availablePositions, availableStages, onUpdateCandidate, onDeleteCandidate, onOpenUploadModal, onEditPosition, isLoading, onRefreshCandidateData, selectedCandidateIds, onToggleSelectCandidate, onToggleSelectAllCandidates, isAllCandidatesSelected, }) {
    ...
    const handleManageTransitionsClick = (candidate) => {
        setSelectedCandidateForModal(candidate);
        setIsTransitionsModalOpen(true);
    };
    const handleEditPositionClick = (positionId) => {
        if (!positionId)
            return;
    ...
    if (isLoading) {
        return (<div className="flex flex-col items-center justify-center h-64 border rounded-lg bg-card shadow">
    ...
    return (<>
        ...
        <Table>
          ...
          <TableBody>
            ...
            if (dateValue && typeof dateValue === 'string') {
                ...
                try {
                    displayDate = format(new Date(dateValue), "MMM d, yyyy");
                }
                catch (e) {
            ...
            return (<TableRow key={candidate.id} className="hover:bg-muted/50 transition-colors" data-state={selectedCandidateIds.has(candidate.id) ? 'selected' : ''}>
                  <TableCell><Checkbox checked={selectedCandidateIds.has(candidate.id)} onCheckedChange={() => onToggleSelectCandidate(candidate.id)} aria-label={`Select candidate ${candidate.name}`}/></TableCell>
                  <TableCell>
                  ...
                  <TableCell className="text-xs hidden sm:table-cell">
                    ...
                    <span className="text-green-600 truncate block max-w-[100px] hover:underline cursor-pointer" title={candidate.resumePath}>
                        {((_d = candidate.resumePath.split('-').pop()) === null || _d === void 0 ? void 0 : _d.split('.').slice(0, -1).join('.')) || candidate.resumePath.split('-').pop()}
                      </span>
                  ...
                  <TableCell className="text-right">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        {candidate.positionId && (<DropdownMenuItem onClick={() => handleEditPositionClick(candidate.positionId)}>
      ...
      {selectedCandidateForModal && (<ManageTransitionsModal candidate={selectedCandidateForModal} isOpen={isTransitionsModalOpen} onOpenChange={(isOpen) => {
                setIsTransitionsModalOpen(isOpen);
                if (!isOpen)
                    setSelectedCandidateForModal(null);
            }} onUpdateCandidate={onUpdateCandidate} onRefreshCandidateData={onRefreshCandidateData} availableStages={availableStages}/>)}
      <AlertDialog open={!!candidateToDelete} onOpenChange={(open) => { if (!open)
        setCandidateToDelete(null); }}>
        <AlertDialogContent>
          <AlertDialogHeader>

...
export async function fetchAllPositionsDb() {
...
export async function fetchAllRecruitmentStagesDb() {
    const redisClient = await getRedisClient();
    if (redisClient) {
        try {
...
export async function fetchAllUsersDb(filterRole) {
    const redisClient = await getRedisClient();
...
export async function fetchInitialDashboardCandidatesDb(limit = 10) {
    const pool = getPool();
    try {
        const query = `
